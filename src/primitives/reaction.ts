...
export class Reaction {
  ...
  constructor(data: { emoji: string; agentId: string; messageId: string }) {
  ...
  get displayEmoji(): string {
    return (SemanticReactions as Record<string, string>)[this.emoji] ?? this.emoji;
  }
...
export class ReactionSummary {
  readonly messageId: string;
  private readonly _reactions: Map<string, string[]> = new Map(); // emoji -> [agentIds]
  ...
  toJSON() {
    const reactions: Record<string, { display: string; count: number; agents: string[] }> = {};

    for (const [emoji, agents] of this._reactions) {
      reactions[emoji] = {
        display: (SemanticReactions as Record<string, string>)[emoji] ?? emoji,
        count: agents.length,
        agents,
      };
    }
...
export class ReactionManager {
  private readonly _reactions: Map<string, ReactionSummary> = new Map();
  private readonly _history: Reaction[] = [];
...
export const acknowledge = (messageId: string, agentId: string) =>
  new Reaction({ emoji: 'eyes', agentId, messageId });

export const approve = (messageId: string, agentId: string) =>
  new Reaction({ emoji: 'thumbsup', agentId, messageId });

export const complete = (messageId: string, agentId: string) =>
  new Reaction({ emoji: 'white_check_mark', agentId, messageId });

export const workingOn = (messageId: string, agentId: string) =>
  new Reaction({ emoji: 'hourglass', agentId, messageId });

export const alert = (messageId: string, agentId: string) =>
  new Reaction({ emoji: 'rotating_light', agentId, messageId });
