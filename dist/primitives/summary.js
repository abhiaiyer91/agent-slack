 ...
 */
function extractTopics(messages, maxTopics) {
    const wordFreq = {};
 ...
 */
function generateTextSummary(channelName, participants, messageCount, topics, decisions, openQuestions, sentiment) {
    const parts = [];
 ...
 */
export function generateChannelSummary(channel, messages, options) {
    ...
    const textSummary = config.generateTextSummary
        ? generateTextSummary(channel.name, activeParticipants.map((p) => p.agentId), messages.length, topics, decisions, openQuestions, sentiment)
        : '';
    return {
        id: nanoid(),
...
export class SummaryManager {
     ...
     */
    generate(channel, messages, options) {
