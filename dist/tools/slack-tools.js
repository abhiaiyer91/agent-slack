 ...
 */
export function createSlackTools(workspace, agentId) {
    // ==================== Messaging Tools ====================
    const postMessage = createTool({
        ...
        execute: async ({ context }) => {
            ...
            const message = workspace.postMessage(agentId, channel.id, context.content, { messageType: context.messageType ?? MessageType.TEXT });
            return {
    ...
    const replyInThread = createTool({
        ...
        execute: async ({ context }) => {
            const reply = workspace.replyInThread(agentId, context.parentMessageId, context.content);
            return {
                messageId: reply.id,
...
Please react with 👀 to acknowledge or ❌ to decline.`;
    ...
    const setPresence = createTool({
        ...
        execute: async ({ context }) => {
            const presence = workspace.setPresence(agentId, context.status, context.message ?? '');
            return {
 ...
 */
export function getSlackToolsArray(workspace, agentId) {
